	...
	if os.Getenv("SEARCH_BACKEND") == "MYSQL" {
		...
		if DB != nil {
			...
			DB.Close()
                ...
                preFile, err := os.Open("/opt/registry/repositories")
                if err == nil {
			preBuf := bufio.NewReader(preFile)
                        for {
				repoPath, err := preBuf.ReadString('\n')
		...
		preFile.Close()
		_, exists := repos[ts.repository.Named().Name()]
		if !exists {
			file, err := os.OpenFile("/opt/registry/repositories", os.O_CREATE|os.O_APPEND|os.O_RDWR, 0666)
			if err == nil {
				io.WriteString(file, ts.repository.Named().Name() + "\n")
			}
			file.Close()
		}
	}
